...
export function normalizeProfile(raw) {
  ...
  return {
    ...
    // One pattern per profile: the policy Apply fetches is installed on every URL the
    // profile matches, so a second unrelated pattern would receive the first site's
    // CSP. A config written before this rule may hold more; only the first is kept,
    // and the popup reports the rest rather than dropping them silently.
    matches: Array.isArray(raw.matches)
      ? raw.matches.filter((m) => typeof m === 'string' && m.trim()).slice(0, 1)
      : [],
...
export function profileIssues(profile) {
  ...
  if (profile.matches.length === 0) {
    errors.push('No match pattern — add the site this should run on.');
  }
  for (const pattern of profile.matches) {
